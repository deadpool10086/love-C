...
void FillRandom(char *buf, int len)
{
	for(int i = 0; i < len; i++)
	{
		buf[i] = rand();
...
void EncRound(char *plain, int plainlen, char *cipher, char *IV, char *key, int keylen)
{
	for(int i = 0; i < plainlen; i++)
	{
		...
		{
			cipher[i] = EncByte(cipher[i - 1], plain[i], key[i % keylen]);
		}
	}
}
...
void Encrypt(char *plain, int plainlen, char *cipher, char *key, int keylen, int round)
{
	for(int r = 0; r < round; r++)
	{
		if(r == 0)
		{
			EncRound(plain, plainlen, cipher, &key[0], &key[1], keylen - 1);
		}
		else
		{
			EncRound(cipher, plainlen, cipher, &cipher[plainlen - 1], &key[1], keylen - 1);
		}
	}
}
...
void Decrypt(char *cipher, int cipherlen, char *plain, char *key, int keylen, int round)
{
	for(int r = round - 1; r >= 0; r--)
	{
		if(r == 0)
		{
			DecRound(cipher, cipherlen, plain, &key[0], &key[1], keylen - 1);
		}
		else
		{
...
#define sample1_keyround	1
#define sample1_keylen		strlen(sample1_key)
...
int main()
{
	for (int i=0; i<45; i++)
	{
		printf("0x%X  ",(unsigned char)rsbox[sample1_cipher[i]]);
	}
	return 0;
} 
